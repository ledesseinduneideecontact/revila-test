...
function validatePromoCode(code: string | null): { isValid: boolean; discount: number; message: string } {
  if (!code) {
  ...
  switch(upperCode) {
    case 'PETITEFLEUR':
      return { isValid: true, discount: 0.10, message: 'Code promo PETITEFLEUR valide - 10% de réduction' }
    case 'REVIVE10':
      return { isValid: true, discount: 0.10, message: 'Code promo REVIVE10 valide - 10% de réduction' }
    case 'PARRAIN20':
      return { isValid: true, discount: 0.20, message: 'Code parrainage valide - 20% de réduction' }
    case 'NOEL':
    case 'NOEL2025':
      return { isValid: true, discount: 0.15, message: 'Code promo NOEL valide - 15% de réduction' }
    default:
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    while (true) {
      ...
      try {
        // Upload vers Google Cloud Storage (GCS)
        ...
        if (hasValidPhoto) {
          console.log(`   - Photo path: ${photoPath} (${(photoFile.size/1024/1024).toFixed(1)} Mo, ${photoFile.type})`)
          const photoBuffer = Buffer.from(await photoFile.arrayBuffer())
          photoUrl = await uploadToGCS(photoBuffer, photoPath!, photoFile.type)
          console.log(`   ✅ Photo uploaded to GCS: ${photoUrl}`)
        }

        if (hasValidVideo) {
          console.log(`   - Video path: ${videoPath} (${(videoFile.size/1024/1024).toFixed(1)} Mo, ${videoFile.type})`)
          const videoBuffer = Buffer.from(await videoFile.arrayBuffer())
          videoUrl = await uploadToGCS(videoBuffer, videoPath!, videoFile.type)
          console.log(`   ✅ Video uploaded to GCS: ${videoUrl}`)
        }

        console.log(`📸 Creating order item ${position} with quantity ${photoQuantity}...`)
        ...
        const { error: itemError } = await supabaseAdmin
          ...
          .insert({
            ...
            categorie: `photo-${photoSize}`, // Ajouter la catégorie selon le format
            quantity: photoQuantity, // Utiliser la quantité envoyée par le frontend
            photo_gcs_url: photoUrl,
            video_gcs_url: videoUrl,
            photo_filename: photoFileName,
            video_filename: videoFileName,
            photo_size_bytes: hasValidPhoto ? photoFile.size : null,
            video_size_bytes: hasValidVideo ? videoFile.size : null,
            photo_mime_type: hasValidPhoto ? photoFile.type : null,
            video_mime_type: hasValidVideo ? videoFile.type : null,
            unit_price: subtotalAmount / (itemIndex + 1) / photoQuantity, // Prix unitaire
            unit_price_cents: Math.round((subtotalAmount / (itemIndex + 1) / photoQuantity) * 100),
            line_total_cents: Math.round((subtotalAmount / (itemIndex + 1)) * 100), // Prix total pour toutes les copies
            gcs_upload_status: 'uploaded',
            // Ajouter les champs manquants
            message_text: message || null,
            message_signature: signature || null,
            cadeau: isGift,
            nom: recipientName || null,
            adresse: recipientAddress || null,
            code_postal: recipientPostalCode || null,
            ville: recipientCity || null,
            pays: 'France'
          })

        if (itemError) {
          console.error(`❌ Order item ${position} creation error:`, itemError)
          // Supprimer la commande si un item échoue
          await supabaseAdmin.from('orders').delete().eq('id', order.id)
          const message = (itemError as any)?.message || 'Insert order_items error'
          const details = (itemError as any)?.details || ''
          throw new Error(`Order item insert failed: ${message} ${details}`)
        }
        console.log(`✅ Order item ${position} created with quantity ${photoQuantity}`)
        totalOrderItems++ // Incrémenter le compteur
        ...
        // Incrémenter les compteurs seulement si les fichiers ont été uploadés
        if (hasValidPhoto) photoCounter++
        if (hasValidVideo) videoCounter++
        
      } catch (uploadError) {
        console.error(`Upload error for item ${position}:`, uploadError)
        // Si l'upload échoue, supprimer la commande créée
        await supabaseAdmin
          .from('orders')
          .delete()
          .eq('id', order.id)
        throw new Error(`Erreur upload GCS pour item ${position}: ${uploadError instanceof Error ? uploadError.message : String(uploadError)}`)
      }
